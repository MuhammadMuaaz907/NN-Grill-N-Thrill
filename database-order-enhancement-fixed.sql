-- Professional Order Management Enhancement - FIXED VERSION
...
-- Update existing orders to have proper flags (FIXED VERSION)
UPDATE orders SET 
  is_new = true,
  admin_seen = false,
  priority = CASE 
...
-- Create function to automatically set order priority based on amount (FIXED VERSION)
CREATE OR REPLACE FUNCTION set_order_priority()
RETURNS TRIGGER AS $$
BEGIN
  -- Set priority based on order amount with NULL safety
  IF NEW.totals IS NOT NULL AND NEW.totals->>'grand_total' IS NOT NULL THEN
...
-- Success message
SELECT 'Professional Order Management Enhancement completed successfully!' as message;
